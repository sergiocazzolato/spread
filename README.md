...
## Fetching project-level residual artifacts

Content generated may easily be retrieved after the spread run finishes execution
by registering the desired content under the `project-artifacts` field of the spread.yaml:
...
(contained in the environment variable `$SPREAD_PATH`), and they are only considered 
when Spread is run with the `-project-artifacts` flag pointing
...
## Fetching task-level residual artifacts
